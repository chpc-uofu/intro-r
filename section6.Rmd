...
\item Let $X$, $Y$ be independent random variables $\sim Unif(0,1)$.$\newline$
      Then $Z=X+Y$ has the following pdf: \href{run:./latex/notes.pdf}{(If you are interested in the details, click here)} 
      \begin{eqnarray}
        f_Z(z) = \begin{cases}
                    z &,\; 0 \le z \le 1 \\
                    2 - z &,\; 1 \le z \le 2 \nonumber
                 \end{cases}
      \end{eqnarray}
      Generate the vectors x and y each having $10^5$ random numbers $\sim Unif(0,1)$.$\newline$
      Use the $\textcolor{blue}{\textbf{hist()}}$ function to plot the $z=x+y$ vector.
